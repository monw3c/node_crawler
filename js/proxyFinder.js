...
function getRequest(url,callback,charset='utf8'){
    ...
    .charset(charset).set({
                'Referer': url,
                'User-Agent': "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.106 Safari/537.36",
...
function check_https(proxy){
    var url = 'https://jinshuju.net/f/YqBTlv';
    var proxy = 'http://'+proxy;
...
var init = async function (){
        ...
        await clearLog();
        await loadLog();
        await console.log("Successfully load log")
        await getProxy()
